...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def union(that: TweetSet): TweetSet = {
    ...
    //((right.union(that)).union(left)).incl(elem)
  }
  ...
  def mostRetweeted: Tweet = {
    if (left.isEmpty && right.isEmpty) elem
      ...
      val most =if (left.isEmpty) right.mostRetweeted
      else if (right.isEmpty) left.mostRetweeted
      else {
...
object GoogleVsApple {
  ...
  def contain(s: String, key: List[String]): Boolean = {
    if (key.isEmpty) false
    else s.contains(key.head) || contain(s, key.tail)
  }

    lazy val googleTweets: TweetSet =
      TweetReader.allTweets.filter((x: Tweet) => google.exists(y => (x.text.contains(y))))
      //TweetReader.allTweets.filter((x: Tweet) => contain(x.text, google))

  lazy val appleTweets: TweetSet =
    TweetReader.allTweets.filter((x: Tweet) => apple.exists(y => (x.text.contains(y))))
  //TweetReader.allTweets.filter((x: Tweet) => contain(x.text, apple))
